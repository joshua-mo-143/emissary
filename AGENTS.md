...
**Postcode/location gates:** for takeaway or delivery pages that only need a postcode/postal code to show restaurants or availability, use `submitPostcode` with the vault profile and `kind: "shipping"` instead of `type`/`typeRef`. The postcode comes from the 1Password-backed shipping address (falling back to the profile postal code), is redacted from browser text output, and the action may only click guarded non-final search/continue controls.
